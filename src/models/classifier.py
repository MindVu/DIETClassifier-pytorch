...
# from transformers import BertForTokenClassification, BertPreTrainedModel, BertModel, AutoModelForTokenClassification, AutoModel
...
#     sentences = ["tôi không muốn xem ti vi nữa"]
...
class DIETClassifier(AutoModelForTokenClassification):
    def __init__(self, config: DIETClassifierConfig):
        if path.exists(config.model):
            try:
                json_config = json.load(open(f"{config.model}/config.json", "r"))
            ...
            pretrained_model = AutoModelForTokenClassification.from_pretrained(config.model)
            checkpoint = None
            if config.intents is None or config.entities is None:
                raise ValueError(f"Using pretrained from transformers should specific entities and intents")
            pretrained_model.config.update({"model": config.model, "entities": config.entities, "intents": config.intents})
        ...
        self.bert = AutoModel.from_pretrained(config.model) if not pretrained_model else pretrained_model.bert
